...
| `src/lib.rs` | this crate's own registry | 65 |

After de-duplication that yields **485 modules** (`node scripts/build-inventory.mjs`
...
actually asserts. Current split: **65 `verified`** (the whole registry), 420 `catalogued`.
...
2. Every registered kernel now has a numeric reference. Next is breadth: work down the
   catalogue in priority order: colour/tone (50), composite/geometry (43), blend (35),
   draw/text (26), blur/sharpen (25), edge/detail (12), morphology (10).
